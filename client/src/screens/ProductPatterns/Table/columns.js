...
const roundValue = (value, suffix, color) => {
  let firstValue = value < 0 ? 0 : value;
  let firstValueStr = `${firstValue * 100}`;
  let containsDot = firstValueStr.indexOf(".") !== -1;
  if (containsDot) {
    firstValueStr = firstValueStr.substring(0, firstValueStr.indexOf("."));
  }
  let localValue = firstValueStr;
  return (
